...
@Component({
    moduleId: module.id,
    selector: 'fileupload-patient-list',
    templateUrl: 'fileupload-patient.component.html'
    //styleUrls: ['./patient.component.css'] 
})

export class FileUploadPatientComponent implements OnInit {
    ...
    public listResult: PatientFileModel[];
    ...
    goBackToList() {
        let navigateUrl = 'patient/manage';
        this.router.navigate([navigateUrl, { parentId: this.parentId }]);
    }
    newRegister(): void { 
        let navigateUrl = 'patient/manage/fileaction';
        this.router.navigate([navigateUrl, { parentId: this.getPatientId() }]);
    }
    downloadRegister(idRegister: number): void {
        ...
        this.registerService.downloadFile(idRegister).subscribe({
            next: (response: HttpResponse<Blob>) => {
                ...
                //CaptureTologFunc('downloadFile-patient', response);
            },
    ...
    viewRegister(idRegister: number): void {
        ...
        if (userLogger.typeUser === "Admin")
            navigateUrl = '/patient/manage/fileaction';

        if (userLogger.typeUser === "patient")
            navigateUrl = '/patient/managefiles/fileaction';
    ...
    private getPatientId(): number {
        ...
        let patientId: number = Number(paramsUrl.get('parentId'));
        return patientId;
    }
    retrieveList(): void {
    ...
    executeDeleteRegister(idRegister: number) {
        this.registerService.delete(idRegister).subscribe({
            next: (response: any) => {
                ...
                this.listResult = this.removeItemFromList<PatientFileModel>(this.listResult, idRegister);
